...
Branch_Predictor *initBranchPredictor()
{
    ...
    int i = 0;
    signed int *zero_array1 = malloc(globalCounterBits * sizeof(signed int));
    signed int *zero_array2 = malloc(globalCounterBits * sizeof(signed int));
    for (i = 0; i < globalCounterBits; i++)
    {
        zero_array1[i] = 1;
        zero_array2[i] = 1;
    }
    branch_predictor->global_history_table = zero_array1;

    for (i = 0; i < globalPredictorSize; i++)
    {
        branch_predictor->perceptron_table[i].weight = zero_array2;
        branch_predictor->perceptron_table[i].y_out = 0;
    }
...
bool predict(Branch_Predictor *branch_predictor, Instruction *instr)
{
    ...
    //uint64_t branch_address_masked = branch_address & branch_predictor->global_history_mask;
    //uint64_t table_index = branch_address_masked ^ global_masked;
    unsigned table_index = getIndex(branch_address, branch_predictor->global_history_mask);

    Perceptron perceptron = branch_predictor->perceptron_table[table_index];
    perceptron.y_out += perceptron.weight[0];

    int i = 0;
    for (i = 1; i < globalCounterBits; i++)
    {
        perceptron.y_out += branch_predictor->global_history_table[i] * perceptron.weight[i];
    }
    if (perceptron.y_out < 0)
    {
        prediction = 0;
    }
    else
    ...
    float theta = (1.93 * globalCounterBits) + 14;

    if (((perceptron.y_out < 0) == instr->taken) || (abs(perceptron.y_out) <= theta))
    {
        for (i = 0; i < globalCounterBits; i++)
        {
            if (instr->taken)
            {
                perceptron.weight[i] += branch_predictor->global_history_table[i];
            }
            else
            {
                perceptron.weight[i] -= branch_predictor->global_history_table[i];
    ...
    for (i = globalCounterBits-1; i > 1; i--)
    {
        branch_predictor->global_history_table[i] = branch_predictor->global_history_table[i-1];
    }

    if (instr->taken)
    {
        branch_predictor->global_history_table[1] = 1;
    }
    else
    {
        branch_predictor->global_history_table[1] = -1;
